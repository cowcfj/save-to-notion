...
#### 🚀 核心功能 1：保存後打開 Notion 頁面
- **一鍵訪問**：保存成功後自動顯示「🔗 Open in Notion」按鈕
- **新標籤打開**：點擊後在新標籤頁中打開對應的 Notion 頁面
- **智能顯示**：僅在頁面已保存到 Notion 時顯示
- **用戶價值**：極大提升工作流效率，無需手動在 Notion 中搜索頁面

#### 🎨 核心功能 2：按鈕顯示保存狀態
- **狀態識別**：自動檢測頁面是否已保存到 Notion
- **視覺反饋**：
  - 已保存：按鈕變為綠色，文字顯示「✅ Saved - Save Again」
  - 未保存：按鈕為藍色，文字顯示「💾 Save to Notion」
- **防止重複**：清楚顯示保存狀態，避免不必要的重複保存
- **動態更新**：保存操作完成後立即更新狀態顯示

#### 技術實現
- **popup.html**：新增「Open in Notion」按鈕
- **popup.js**：
  - 重構 `checkPageStatus()` - 智能狀態檢測
  - 新增 `updateUIForSavedPage()` - 已保存狀態 UI
  - 新增 `updateUIForUnsavedPage()` - 未保存狀態 UI
  - 新增 `openNotionButton` 事件處理 - 打開頁面功能
  - 保存成功後自動調用 `checkPageStatus()` 更新 UI
- **popup.css**：
  - 新增 `.saved` 類樣式（綠色主題）
