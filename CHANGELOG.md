...
#### imageUtils.js 安全增強與文檔完善
- **遞歸深度限制**：
  - `cleanImageUrl()` 添加遞歸深度參數（最大深度 10）
  - 防止惡意構造的嵌套代理 URL 導致堆疊溢出
  - 超過深度限制時拋出明確錯誤並記錄日誌
  - 新增常數 `MAX_RECURSION_DEPTH: 10`

- **正則表達式安全文檔**：
  - 為所有正則表達式添加詳細的行內註釋
  - 說明每個正則的用途、模式結構與風險評估
  - 包含 15+ 個正則表達式的完整文檔
  - 涵蓋：協議檢查、路徑模式、排除規則、特殊字符、srcset 解析等
  - 標註所有正則的時間複雜度與 ReDoS 風險評估
...
- **防止無限遞歸**：代理 URL 解析深度限制保護
- **ReDoS 防護持續**：所有正則表達式經過風險評估與文檔
- **安全日誌**：遞歸深度超限時使用 Logger.error() 記錄

### 📊 影響範圍

- **功能無變更**：本次為安全增強與文檔改進，不影響用戶可見功能
- **向後兼容**：完全兼容 v2.11.1 及更早版本
- **測試通過**：✅ 104/104 測試用例通過（100% 通過率）
- **可維護性提升**：正則表達式文檔完整，便於未來維護

### 🧪 測試驗證

- ✅ `tests/unit/imageUtils.test.js`：49/49 測試通過
- ✅ `tests/unit/imageUtils.boundary.test.js`：55/55 測試通過
- ✅ 測試執行時間：0.492 秒
- ✅ 全專案測試保持 100% 通過率
