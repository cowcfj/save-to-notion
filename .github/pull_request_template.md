...
- [ ] **Bundle 體積評估**: 本 PR 是否涉及大型依賴或核心打包結構變动？雖然 CI 的 Hard Size Gate 依然會把關，但若有重大體積變動，建議手動在本地執行 `npm run ext` 評估 unpacked 體積。
- [ ] **Manual heavy gate**: 若本 PR 修改 `scripts/`、`pages/`、`tests/`、Jest config、CI workflow、`package.json` 或 `package-lock.json`，合併前已批准 `manual-heavy-gate` 並確認 `Jest Full Coverage` 通過；release-please / docs-only PR 可標記不適用。
