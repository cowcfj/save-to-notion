...
class TestReport {
  ...
  finalize() {
    ...
    this.summary.passed = this.results.filter(result => result.status === 'passed').length;
    this.summary.failed = this.results.filter(result => result.status === 'failed').length;
    this.summary.skipped = this.results.filter(result => result.status === 'skipped').length;
  }
