  ...
  describe('getFloatingRailCSS', () => {
    ...
    test('reduced-motion 規則順序應晚於所有常規 transition 規則', () => {
      ...
      const transitionSelectors = [
        ...
        '.rail-container.expanded .rail-actions, .rail-container.highlighting .rail-actions',
        '.rail-trigger',
        '.rail-action-btn',
      ...
      for (const selector of transitionSelectors) {
        ...
        const reducedRule = findRuleBlock(normalized, selector, block =>
          /transition:\s*none/.test(block)
        );

        expect(regularRule.index).toBeGreaterThan(-1);
        expect(reducedRule.index).toBeGreaterThan(-1);
        expect(reducedRule.index).toBeGreaterThan(regularRule.index);
      }
    });
  });
