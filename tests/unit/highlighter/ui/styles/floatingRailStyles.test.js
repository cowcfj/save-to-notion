...
describe('floatingRailStyles', () => {
  describe('getFloatingRailCSS', () => {
    ...
    test('reduced-motion 規則順序應晚於常規 collapsed 狀態 transition 規則', () => {
      ...
      const regularCollapsedTransition =
        '.rail-container.collapsed .rail-actions { max-height: 0; opacity: 0; overflow: hidden; padding: 0; transition: max-height 0.2s ease, opacity 0.15s ease; }';
      const reducedCollapsedTransition =
        '.rail-container.collapsed .rail-actions { transition: none; }';

      const regularCollapsedIndex = normalized.indexOf(regularCollapsedTransition);
      const reducedCollapsedIndex = normalized.lastIndexOf(reducedCollapsedTransition);

      // 常規 transition 必須先宣告，reduced-motion override 必須後宣告才能在同 specificity 下勝出
      expect(regularCollapsedIndex).toBeGreaterThan(-1);
      expect(reducedCollapsedIndex).toBeGreaterThan(-1);
      expect(reducedCollapsedIndex).toBeGreaterThan(regularCollapsedIndex);
    });
  });
