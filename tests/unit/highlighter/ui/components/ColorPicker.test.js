...
describe('ColorPicker', () => {
    ...
    describe('渲染', () => {
        test('應該渲染所有顏色按鈕', () => {
            ...
            const buttons = container.querySelectorAll('.color-btn-v2');
        ...
        test('應該為當前顏色添加選中樣式', () => {
            ...
            // 瀏覽器會將 #333 轉換為 rgb 格式
            expect(yellowBtn.style.border).toMatch(/3px solid (rgb\(51, 51, 51\)|#333)/);
        });
        ...
        test('應該為非當前顏色添加未選中樣式', () => {
            ...
            // 瀏覽器會將 #ddd 轉換為 rgb 格式
            expect(greenBtn.style.border).toMatch(/2px solid (rgb\(221, 221, 221\)|#ddd)/);
        });
    ...
    describe('交互', () => {
        ...
        test('應該在點擊後重新渲染以更新選中狀態', () => {
            ...
            // 重新渲染後，green 應該被選中
            const updatedGreenBtn = container.querySelector(
                '[data-color="green"]'
            );
            expect(updatedGreenBtn.style.border).toMatch(/3px solid (rgb\(51, 51, 51\)|#333)/);
        });
    });
});
