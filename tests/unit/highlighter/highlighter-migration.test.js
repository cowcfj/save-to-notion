...
describe('HighlightMigrationManager', () => {
  ...
  describe('convertColorToName', () => {
    test('應該轉換 RGB 顏色值', () => {
      expect(window.HighlightMigrationManager.convertColorToName('rgb(255, 243, 205)')).toBe(
        'yellow'
      );
      expect(window.HighlightMigrationManager.convertColorToName('rgb(212, 237, 218)')).toBe(
        'green'
      );
      expect(window.HighlightMigrationManager.convertColorToName('rgb(204, 231, 255)')).toBe(
        'blue'
      );
  ...
  describe('showMigrationResult', () => {
    test('應該顯示成功的遷移結果', () => {
      ...
      const notification = Array.from(notifications).find(el => el.innerHTML?.includes('遷移完成'));
