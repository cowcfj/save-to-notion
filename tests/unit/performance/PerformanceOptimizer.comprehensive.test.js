    ...
    test.each([
      {
        ...
        options: {},
        createCustomBatchFn: () =>
          jest.fn((items, processor) => Promise.resolve(items.map(item => processor(item)))),
      },
      {
        name: '失敗後重試並最終成功',
        items: [1, 2, 3],
        ...
        createCustomBatchFn: () => {
          let attempt = 0;
          return jest.fn((items, processor) => {
            attempt++;
            if (attempt === 1) {
      ...
      expect(meta.attempts).toBe(scenario.expectedAttempts);
