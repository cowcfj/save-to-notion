...
describe('FallbackStrategies - 全面測試', () => {
  ...
  describe('extractFromPicture', () => {
    ...
    it('應該處理多個 source 元素並返回第一個有效的', () => {
      ...
      // 更新：現在使用 ImageUtils 進行驗證，會正確跳過無效的 URL
  ...
  describe('extractFromNoscript', () => {
    ...
    it('應該處理複雜的 HTML 內容', () => {
      ...
      const noscript = document.createElement('noscript');
      noscript.innerHTML =
        '<div><img class="lazy" src="https://example.com/complex.jpg" alt="test"></div>';
      div.appendChild(noscript);
  ...
  describe('_extractUrlFromSrcset', () => {
    ...
    it('應該從多個條目中提取第一個有效的 URL', () => {
      const result = FallbackStrategies._extractUrlFromSrcset(
        'https://example.com/valid.jpg 1200w, https://example.com/another.jpg 800w'
      );
    ...
    it('應該修剪空白', () => {
      const result = FallbackStrategies._extractUrlFromSrcset(
        '  https://example.com/image.jpg  800w  '
      );
