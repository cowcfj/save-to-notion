...
const {
  ExtractionStrategy,
  ExtractionResult,
} = require('../../../scripts/imageExtraction/ExtractionStrategy');
...
describe('ExtractionStrategy - 全面測試', () => {
  ...
  describe('ExtractionStrategy 基類', () => {
    describe('extract()', () => {
      ...
      it('應該要求子類實現', () => {
        expect(() => strategy.extract(imgElement)).toThrow(
          'ExtractionStrategy.extract() must be implemented by subclass'
        );
      });
    });
    ...
    describe('getName()', () => {
      it('應該要求子類實現', () => {
        expect(() => strategy.getName()).toThrow(
          'ExtractionStrategy.getName() must be implemented by subclass'
        );
      });
    });
    ...
    describe('_cleanUrl()', () => {
      it('應該修剪空白字符', () => {
        expect(strategy._cleanUrl('  https://example.com/image.jpg  ')).toBe(
  ...
  describe('ExtractionResult', () => {
    describe('構造函數', () => {
      it('應該創建基本結果', () => {
        const result = new ExtractionResult('https://example.com/image.jpg', 'src-attribute');
      ...
      it('應該支持自定義置信度', () => {
        const result = new ExtractionResult('https://example.com/image.jpg', 'fallback', 0.7);

        expect(result.confidence).toBe(0.7);
      });

      it('應該支持自定義元數據', () => {
        const result = new ExtractionResult('https://example.com/image.jpg', 'srcset', 1.0, {
          width: 1200,
          height: 800,
        });
    ...
    describe('isValid()', () => {
      ...
      it('置信度為 0 應該返回 false', () => {
        const result = new ExtractionResult('https://example.com/image.jpg', 'src-attribute', 0);

        expect(result.isValid()).toBe(false);
      });

      it('負置信度應該返回 false', () => {
        const result = new ExtractionResult('https://example.com/image.jpg', 'src-attribute', -0.5);
    ...
    describe('toJSON()', () => {
      it('應該返回完整的 JSON 對象', () => {
        const result = new ExtractionResult('https://example.com/image.jpg', 'src-attribute', 0.9, {
          width: 800,
        });
  ...
  describe('子類實現示例', () => {
    ...
    it('子類應該繼承父類的工具方法', () => {
      ...
      expect(testStrategy._cleanUrl('"https://example.com/image.jpg"')).toBe(
        'https://example.com/image.jpg'
      );
    });
