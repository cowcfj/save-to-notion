...
const ACTION_MODULE_EXPECTATIONS = [
  {
    registryName: 'PRELOADER_ACTIONS',
    keys: ['USER_ACTIVATE_SHORTCUT', 'PING', 'INIT_BUNDLE', 'REPLAY_BUFFERED_EVENTS'],
  },
  {
    registryName: 'CONTENT_BRIDGE_ACTIONS',
    keys: [
      'PING',
      'SET_STABLE_URL',
      'GET_STABLE_URL',
  ...
  {
    registryName: 'DIAGNOSTICS_ACTIONS',
    keys: ['DEV_LOG_SINK', 'DEV_LOG_SINK_BATCH', 'EXPORT_DEBUG_LOGS'],
  },
  {
    registryName: 'HIGHLIGHTER_ACTIONS',
    keys: [
      'SHOW_TOOLBAR',
      'SHOW_HIGHLIGHTER',
      'START_HIGHLIGHT',
  ...
  {
    registryName: 'PAGE_SAVE_ACTIONS',
    keys: [
      'CHECK_PAGE_STATUS',
      'SAVE_PAGE',
  ...
  {
    registryName: 'MIGRATION_ACTIONS',
    keys: [
      'MIGRATION_EXECUTE',
      'MIGRATION_DELETE',
      'MIGRATION_BATCH',
  ...
  {
    registryName: 'DRIVE_SYNC_ACTIONS',
    keys: [
...
describe('runtime action 模組拆分', () => {
  test('拆分模組應輸出凍結的 action registry，且值與 aggregate registry 一致', () => {
    ...
    for (const { registryName, keys } of ACTION_MODULE_EXPECTATIONS) {
      const actionRegistry = actionRegistries[registryName];
