...
describe('cleanImageUrl - 深度測試', () => {
  describe('代理 URL 處理（未覆蓋部分）', () => {
    ...
    test('應該處理帶有額外參數的代理 URL', () => {
      const proxyUrl =
        'https://proxy.example.com/photo.php?u=https://real.example.com/image.jpg&size=large';
      expect(cleanImageUrl(proxyUrl)).toBe('https://real.example.com/image.jpg');
    });
    ...
    test('應該遞歸清理嵌套代理 URL', () => {
      const nestedProxy =
        'https://proxy1.com/photo.php?u=https://proxy2.com/gw/photo.php?u=https://real.com/image.jpg';
