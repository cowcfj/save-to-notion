...
async function testErrorHandling(blocks, errorScenario) {
  ...
  mockNotionAPI.blocks.children.append = jest.fn().mockImplementation(() => {
    ...
    if (Math.random() < errorScenario.failureRate) {
      ...
      return Promise.reject(new Error(`${errorType} Error`));
    }
    return Promise.resolve({ results: [] });
  });
  ...
  try {
      ...
      while (!batchSuccess && attempts < maxRetries) {
        try {
          ...
          if (attempts < maxRetries) {
            // 指數退避（縮短延遲以加快測試）
            const backoffDelay = Math.min(10 * Math.pow(2, attempts), 50);
    ...
    return {
      success: true,
      processingTime: endTime - startTime,
      finalSuccessRate: successfulBatches / batches.length,
      retryCount,
      successfulBatches,
    ...
    return {
      success: false,
      processingTime: endTime - startTime,
      finalSuccessRate: successfulBatches / chunkArray(blocks, 100).length,
      retryCount,
      error: error.message
    };
  }
