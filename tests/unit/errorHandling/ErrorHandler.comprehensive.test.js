...
describe('ErrorHandler - 全面測試', () => {
  ...
  describe('withRetry - 重試機制', () => {
    ...
    test('應該記錄重試嘗試', async () => {
      ...
      const asyncOperation = jest.fn().mockRejectedValueOnce(error).mockResolvedValue('success');
  ...
  describe('createError - 創建標準化錯誤', () => {
    ...
    test('應該使用默認的嚴重程度', () => {
      const error = ErrorHandler.createError(ErrorTypes.PARSING_ERROR, 'Parse failed');

      expect(error.severity).toBe(ErrorSeverity.MEDIUM);
    });

    test('應該使用空對象作為默認詳情', () => {
      const error = ErrorHandler.createError(ErrorTypes.DOM_ERROR, 'DOM operation failed');
  ...
  describe('wrapAsync - 包裝異步函數', () => {
    ...
    test('應該在失敗後重試', async () => {
      ...
      const asyncFn = jest.fn().mockRejectedValueOnce(error).mockResolvedValue('success');
