...
describe('seamless-migration.js', () => {
  beforeEach(() => {
    ...
    mockChrome.storage.local.set.mockImplementation(_data => {
      return Promise.resolve();
    });
    mockChrome.storage.local.remove.mockImplementation(_keys => {
      return Promise.resolve();
    });
  });
  ...
  describe('SeamlessMigrationManager', () => {
    ...
    describe('performSeamlessMigration', () => {
      ...
      test('應該在超過最大重試次數時停止遷移', async () => {
        ...
        const result = await migrationManager.performSeamlessMigration(mockHighlightManager);
        expect(result.error).toBe('Migration failed after max retries');
      });
    });
    ...
    describe('convertColorToName', () => {
      test('應該轉換顏色值', () => {
        expect(window.SeamlessMigrationManager.convertColorToName('rgb(255, 243, 205)')).toBe(
          'yellow'
        );
        expect(window.SeamlessMigrationManager.convertColorToName('rgb(212, 237, 218)')).toBe(
          'green'
        );
