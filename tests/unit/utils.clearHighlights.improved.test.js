...
describe('StorageUtil.clearHighlights - 改進版測試', () => {
    ...
    describe('輸入驗證', () => {
        ...
        test('應該拒絕非字串類型的 URL', async () => {
            await expect(StorageUtil.clearHighlights(123))
                .rejects.toThrow('Invalid pageUrl: must be a non-empty string');
        ...
        test('應該處理 URL 標準化失敗', async () => {
            ...
            // 這是現有實現的行為，我們保持向後兼容
            await expect(StorageUtil.clearHighlights(invalidUrl))
    ...
    describe('錯誤處理', () => {
        ...
        test('當 localStorage 失敗但 Chrome Storage 成功時應該記錄警告', async () => {
            ...
            StorageUtil._clearFromLocalStorage = jest.fn().mockRejectedValue(
                new Error('localStorage operation failed: mock error')
            );
        ...
        test('當所有存儲清除都失敗時應該拋出錯誤', async () => {
            ...
            StorageUtil._clearFromChromeStorage = jest.fn().mockRejectedValue(
                new Error('Chrome storage error: mock error')
            );
            StorageUtil._clearFromLocalStorage = jest.fn().mockRejectedValue(
                new Error('localStorage operation failed: mock error')
            );

            await expect(StorageUtil.clearHighlights(testUrl))
                .rejects.toThrow('Failed to clear highlights from all storage locations');
    ...
    describe('輔助方法測試', () => {
        describe('_clearFromChromeStorage', () => {
            ...
            test('當 Chrome Storage 不可用時應該拋出錯誤', async () => {
                ...
                await expect(StorageUtil._clearFromChromeStorage('test_key'))
            ...
            test('當操作失敗時應該拋出包含錯誤信息的錯誤', async () => {
                ...
                await expect(StorageUtil._clearFromChromeStorage('test_key'))
                    .rejects.toThrow('Chrome storage error: Test error');
            });
        });
        ...
        describe('_clearFromLocalStorage', () => {
            ...
            test('當操作失敗時應該拋出包含錯誤信息的錯誤', async () => {
                ...
                // 正常清除應該成功
                await expect(StorageUtil._clearFromLocalStorage(testKey))
                    .resolves.toBeUndefined();
