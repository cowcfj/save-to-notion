...
const {
  ...
  extractImageSrc,
  isContentGood,
} = require('../helpers/content.testable');
...
describe('content.js - 圖片處理函數', () => {
  describe('cleanImageUrl', () => {
    ...
    test('應該遞歸處理嵌套的代理 URL', () => {
      const nestedProxy =
        'https://proxy1.com/photo.php?u=https://proxy2.com/gw/?u=https://example.com/image.jpg';
  ...
  describe('extractImageSrc - 使用 JSDOM', () => {
    ...
    test('應該從背景圖回退提取（當 img 無有效屬性）', () => {
      ...
      Object.defineProperty(img, 'style', {
        value: { backgroundImage: 'url(https://example.com/bg.jpg)' },
        writable: true,
      });
    ...
    test('應該在 srcset 以最大寬度選擇 URL', () => {
      ...
      img.setAttribute(
        'srcset',
        'https://example.com/a.jpg 400w, https://example.com/b.jpg 800w, https://example.com/c.jpg 1200w'
      );
    ...
    test('應該優先使用 srcset', () => {
      ...
      img.setAttribute(
        'srcset',
        'https://example.com/medium.jpg 800w, https://example.com/large.jpg 1200w'
      );
  ...
  describe('cachedQuery 回退函數', () => {
    ...
    test('應該在 PerformanceOptimizer 可用時使用其 cachedQuery', () => {
      ...
      expect(performanceOptimizer.cachedQuery).toHaveBeenCalledWith('div', document, {
        single: true,
      });
    });
