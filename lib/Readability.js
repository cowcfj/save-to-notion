...
function Readability(doc, options) {
  ...
  this._keepClasses = Boolean(options.keepClasses);
  this._serializer =
    options.serializer ||
    function (el) {
      return el.innerHTML;
    };
  this._disableJSONLD = Boolean(options.disableJSONLD);
  ...
  if (this._debug) {
    /**
     * 將 DOM 節點格式化為可讀的字串（供除錯輸出）。
     * @param {Node} node - 要描述的節點。
     * @returns {string} 節點的可讀描述。
     */
    ...
    const self = this; // Save reference to avoid 'this' binding issues in arrow functions
    this.log = function () {
      if (typeof console !== 'undefined') {
        let args = Array.from(arguments, arg => {
          if (arg && arg.nodeType == self.ELEMENT_NODE) {
            return logNode(arg);
          }
          return arg;
        });
        ...
        // Use indirect reference to avoid static analysis warnings
        // This log is only executed when debug option is explicitly enabled
...
Readability.prototype = {
  ...
  REGEXPS: {
    ...
    commas: /\u{2c}|\u{60c}|\u{fe50}|\u{fe10}|\u{fe11}|\u{2e41}|\u{2e34}|\u{2e32}|\u{ff0c}/gu,
  ...
  _getAllNodesWithTag(node, tagNames) {
    ...
    // Fallback for environments without querySelectorAll:
    ...
    const safeTags = Array.isArray(tagNames)
      ? Array.from(new Set(tagNames.filter(t => typeof t === 'string' && t.trim().length > 0)))
      : [];
    ...
    // Ensure uniqueness to guard against repeated tag names
    return elements.length <= 1 ? elements : Array.from(new Set(elements));
  },
  ...
  _getArticleTitle() {
    ...
    const titleSeparators = /\||\\|-|–|—|\\\\|\/|>|»/u;
    if (new RegExp(`\\s[${titleSeparators.source}]\\s`, 'u').test(curTitle)) {
      titleHadHierarchicalSeparators = /\s[\\/>»]\s/u.test(curTitle);
      let allSeparators = Array.from(
        origTitle.matchAll(new RegExp(`\\s[${titleSeparators.source}]\\s`, 'gu'))
      );
      ...
      if (wordCount(curTitle) < 3) {
        curTitle = origTitle.replace(
          new RegExp(`^[^${titleSeparators.source}]*[${titleSeparators.source}]`, 'gu'),
          ''
        );
      }
    ...
    if (
      ...
      (!titleHadHierarchicalSeparators ||
        curTitleWordCount !=
          wordCount(origTitle.replace(new RegExp(`\\s[${titleSeparators.source}]\\s`, 'gu'), '')) -
            1)
    ) {
      curTitle = origTitle;
    }
  ...
  _markDataTables(root) {
    ...
    for (var i = 0; i < tables.length; i++) {
      ...
      var descendantExists = function (tag) {
        return Boolean(table.getElementsByTagName(tag)[0]);
      };
