...
(function () {
          ...
          if (isLoggerAvailable() && typeof window.Logger[methodName] === 'function') {
            ...
            if (typeof console !== 'undefined' && typeof console[methodName] === 'function') {
              const prefix =
                methodName === 'log'
                  ? '[Highlighter][LOG]'
                  : `[Highlighter][${methodName.toUpperCase()}]`;
  ...
  class HighlightManager {
        ...
        for (const highlightData of highlights) {
          ...
          if (highlightData.rangeInfo) {
            ...
            if (Array.isArray(startContainerPath)) {
              ...
              highlightData.rangeInfo.startContainerPath =
                HighlightManager.convertPathToString(startContainerPath);
              highlightData.rangeInfo.endContainerPath =
                HighlightManager.convertPathToString(endContainerPath);
              needsMigration = true;
            }
          }

          // v2.9.12: 增強恢復邏輯 - 實現穩健的 Range 重建
          const range = await this.restoreRangeWithRetry(
            highlightData.rangeInfo,
            highlightData.text
          );
          if (range) {
    ...
    static validateRange(range, expectedText) {
      try {
        ...
        const lengthMatch =
          actualText.length >= expectedTrimmed.length * 0.8 &&
          actualText.length <= expectedTrimmed.length * 1.2;
        const contentMatch = expectedTrimmed.includes(
          actualText.substring(0, Math.min(20, actualText.length))
        );
    ...
    static validateRangeContext(range, rangeInfo) {
      try {
        ...
        const afterText = parentText.substring(
          rangeStart + rangeText.length,
          rangeStart + rangeText.length + 50
        );

        const contextMatches =
          (!rangeInfo.contextBefore || beforeText.includes(rangeInfo.contextBefore)) &&
    ...
    static waitForDOMStability(options = {}) {
      ...
      return new Promise(resolve => {
        ...
        try {
          ...
          logger.log(
            `🔍 [waitForDOMStability] 開始監視 DOM (閾值: ${stabilityThresholdMs}ms, 超時: ${maxWaitMs}ms)`
          );
        } catch (error) {
          // 觀察失敗，清理並返回 false
          cleanup();
  ...
  function initHighlighter() {
    ...
    toolbar
      .querySelector('#close-highlight-v2')
      .addEventListener('click', originalHide, { once: true });
    ...
    function updateHighlightList() {
      ...
      const highlightsHtml = highlights
        .map((h, index) => {
          ...
          const colorName =
            { yellow: '黃', green: '綠', blue: '藍', red: '紅' }[h.color] || h.color;
          return `
