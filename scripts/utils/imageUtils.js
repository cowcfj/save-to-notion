...
(() => {
  // 防止重複初始化
  if (typeof window !== 'undefined' && window.ImageUtils) {
    return;
  }
  if (typeof self !== 'undefined' && self.ImageUtils) {
    return;
  }
  ...
  // 注意：在 IIFE 中，require 可能不可用，除非它是全域的（如在某些測試環境中）
  if (typeof module !== 'undefined' && typeof require !== 'undefined') {
    try {
      const config = require('../config/constants');
      const configValidation = config.IMAGE_VALIDATION || config.IMAGE_VALIDATION_CONSTANTS;
      if (configValidation) {
        Object.assign(IMAGE_VALIDATION, configValidation);
      }
    } catch (_err) {
      // 忽略加載錯誤，保持默認值
    }
