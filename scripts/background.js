...
async function handleSavePage(sendResponse) {
  try {
    ...
    // 注入並執行內容提取
    // Phase 10c-4: 使用 Feature Flag 控制新舊邏輯
    let result = null;

    if (USE_PAGE_CONTENT_SERVICE) {
      // 新邏輯：使用 PageContentService
      try {
        result = await pageContentService.extractContent(activeTab.id);
        Logger.log('✅ [PageContentService] 內容提取成功');
      } catch (error) {
        Logger.warn('⚠️ [PageContentService] 提取失敗，切換到舊邏輯:', error.message);
        result = null; // 觸發 fallback
      }
    }

    // Fallback: 原有 injectWithResponse 邏輯
    if (!result) {
      try {
            ...
            function selectBestIcon(candidates) {
              ...
              const scored = candidates.map(icon => {
                ...
                if (size === 999) {
                  ...
                  score += 300;
                  Logger.log(
                    `  ${icon.url.substring(0, 60)}...: +300 (ideal size: ${size}x${size})`
                  );
                } else if (size > 256) {
                  // 太大（可能影響性能，但質量好）
                  score += 200;
                  Logger.log(
                    `  ${icon.url.substring(0, 60)}...: +200 (large size: ${size}x${size})`
                  );
                } else if (size >= 120) {
                  // 中等尺寸（可接受）
                  score += 100;
                  ...
                  score += 50;
                  Logger.log(
                    `  ${icon.url.substring(0, 60)}...: +50 (small size: ${size}x${size})`
                  );
                }
            ...
            try {
              ...
              if (!finalContent) {
                ...
                if (typeof window.Readability === 'undefined') {
                  ...
                  Logger.log(
                    '🔄 Readability.js not loaded, falling back to CMS-aware extraction...'
                  );
                  // 將使用下面的備用方案邏輯
                } else {
                  try {
              ...
              if (finalContent) {
                ...
                if (featuredImageUrl) {
                  ...
                  const isDuplicate = blocks.some(
                    block =>
                      block.type === 'image' && block.image?.external?.url === featuredImageUrl
                  );
        ...
        sendResponse({
          success: false,
          error: scriptError?.message || 'Content extraction failed',
        });
        return;
      }
    } // End of if (!result) fallback block
