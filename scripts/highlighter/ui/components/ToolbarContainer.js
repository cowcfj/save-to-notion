  ...
  toolbar.innerHTML = `
        ...
        <div style="display: flex; gap: 8px; margin-bottom: 16px;">
            <button id="${TOOLBAR_SELECTORS.TOGGLE_HIGHLIGHT.slice(
              1
            )}" type="button" class="nh-btn nh-btn-primary" title="Ctrl+點擊標註可快速刪除">開始標註</button>
            <button id="${TOOLBAR_SELECTORS.MINIMIZE.slice(
              1
            )}" type="button" class="nh-btn nh-btn-icon" title="最小化" aria-label="最小化標註工具">
                <svg width="14" height="2" viewBox="0 0 14 2" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1 1H13" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                </svg>
            </button>
            <button id="${TOOLBAR_SELECTORS.CLOSE.slice(
              1
            )}" type="button" class="nh-btn nh-btn-icon" title="關閉" aria-label="關閉標註工具">
