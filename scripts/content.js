...
// @SYNC-WITH: scripts/config/patterns.js
...
(async function () {
        ...
        if (textContent) {
          ...
          if (manyLines || hasBr) {
            ...
            const matchCount = lines.reduce(
              (acc, line) =>
                acc +
                (bulletCharRe.test(line) || numberedRe.test(line) || /^[-••]/u.test(line) ? 1 : 0),
              0
            );
      ...
      case 'IMG': {
        ...
        if (src) {
          try {
            ...
            // 使用更嚴格的 Notion 兼容性檢查
            const isCompatible =
              typeof ImageUtils !== 'undefined' && ImageUtils.isNotionCompatibleImageUrl
  ...
  function processImageForCollection(img, index, featuredImage) {
    ...
    try {
      ...
      Logger.log(
        `✓ Collected image ${index + 1}: ${cleanedUrl.substring(0, 80)}... (${width}x${height})`
      );
      return {
        url: cleanedUrl,
        alt: img.alt || '',
  ...
  async function collectAdditionalImages(contentElement) {
    ...
    if (allImages.length < 1) {
      ...
      Logger.log(
        `Filtered ${docImages.length} total images -> ${filteredImages.length} content images (excluded ${docImages.length - filteredImages.length} from non-content areas)`
      );
      ...
      if (typeof batchProcessWithRetry === 'function') {
        ...
        if (results) {
          ...
          if (meta?.failedIndices?.length) {
            Logger.log(
              `Batch processing skipped ${meta.failedIndices.length} images after validation.`
            );
          }
        } else {
          Logger.warn(
            'Batch processing failed after retries, falling back to sequential processing.',
            meta?.lastError
          );
  ...
  async function expandCollapsibleElements(timeout = 300) {
    try {
      ...
      const collapsedEls = Array.from(
        document.querySelectorAll('.collapsed, .collapse:not(.show)')
      );
  ...
  try {
    ...
    try {
      ...
      Logger.warn('Error while expanding collapsible elements before parsing:', error);
    }
    ...
    if (finalContentHtml) {
      Logger.log(
        `✅ Content extraction successful! Content length: ${finalContentHtml.length} characters`
      );
      ...
      if (imageBlocks.length < 5) {
        ...
        const existingUrls = new Set(imageBlocks.map(block => block.image.external.url));

        let addedCount = 0;
        additionalImages.forEach(imgInfo => {
          if (!existingUrls.has(imgInfo.url) && imageBlocks.length + addedCount < 15) {
            // 最多15張圖片
            // 驗證圖片 URL 安全性
            try {
            ...
            addedCount++;
            Logger.log(
              `✓ Added additional image ${addedCount}: ${imgInfo.url.substring(0, 80)}...`
            );
          }
        });
      ...
      const finalImageCount = blocks.filter(block => block.type === 'image').length;
    ...
    Logger.log(
      `- Readability.js: ${article ? 'Found article but failed quality check' : 'Failed to parse'}`
    );
    ...
    Logger.log(
      `- Page text length: ${document.body ? document.body.textContent.length : 0} characters`
    );
            ...
            rich_text: [
              {
                type: 'text',
                text: {
                  content: `Content extraction failed: ${error.message || 'Unknown error'}. Please try again.`,
                },
              },
            ],
          },
              ...
              rich_text: [
                {
                  type: 'text',
                  text: {
                    content: 'Content extraction returned invalid result. Please try again.',
                  },
                },
              ],
            },
            ...
            rich_text: [
              {
                type: 'text',
                text: {
                  content: 'Content extraction completed but no content blocks were generated.',
                },
              },
            ],
          },
            ...
            rich_text: [
              {
                type: 'text',
                text: {
                  content: `Async content extraction failed: ${error.message || 'Unknown error'}`,
                },
              },
            ],
          },
