...
async function resolveWindowIdFromMessage(message) {
  if (message?.tabId) {
    try {
...
export function createSidepanelHandlers() {
  return {
    ...
    async [RUNTIME_ACTIONS.OPEN_SIDE_PANEL](message, sender) {
      ...
      if (windowId) {
        return openSidePanelForWindow(windowId);
      }

      // 慢速路徑：需要非同步查詢 windowId（來自 message.tabId 或當前視窗）。
      // 此路徑的使用者手勢 Token 已在到達此處前丟失，保留為向下相容的 fallback。
      const resolvedWindowId = await resolveWindowIdFromMessage(message);
